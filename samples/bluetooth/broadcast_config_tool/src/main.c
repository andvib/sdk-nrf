...
static void stream_frame_get_and_send(struct stream_index stream_idx)
{
	...
	struct subgroup_bis_info *bis_info =
	...
	for (size_t i = 0; i < brdcst_param->num_subgroups; i++) {
		...
		for (size_t j = 0; j < brdcst_param->subgroups[i].num_bises; j++) {
			uint8_t streamer_idx =
				subgroup_bis_infos[group_index][i].lc3_streamer_idx[j];
...
static int cmd_file_select(const struct shell *shell, size_t argc, char **argv)
{
	...
	LOG_INF("Selecting file %s for stream big: %d sub: %d bis: %d", file_name, big_index,
		sub_index, bis_index);

	ret = lc3_streamer_stream_register(
		file_name, &subgroup_bis_infos[big_index][sub_index].lc3_streamer_idx[bis_index],
		true);
	if (ret) {
	...
	subgroup_bis_infos[big_index][sub_index].frame_size =
