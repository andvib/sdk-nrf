...
static uint8_t lc3_file_idx[CONFIG_BT_ISO_MAX_BIG][CONFIG_BT_BAP_BROADCAST_SRC_SUBGROUP_COUNT]
			   [CONFIG_BT_BAP_BROADCAST_SRC_STREAM_COUNT];

#define LC3_FILE_INDEX_UNUSED 0xFF
...
static void stream_get_frame_and_send(struct stream_index stream_idx)
{
	...
	uint8_t stream_file_idx = lc3_file_idx[stream_idx.lvl1][stream_idx.lvl2][stream_idx.lvl3];
	...
	if (ret == -ENODATA) {
		...
		lc3_file_idx[stream_idx.lvl1][stream_idx.lvl2][stream_idx.lvl3] =
			LC3_FILE_INDEX_UNUSED;
	...
	for (size_t i = 0; i < ARRAY_SIZE(broadcast_param); i++) {
		...
		for (size_t j = 0; j < ARRAY_SIZE(subgroups[i]); j++) {
			...
			for (size_t k = 0; k < ARRAY_SIZE(stream_location[i][j]); k++) {
				...
				lc3_file_idx[i][j][k] = LC3_FILE_INDEX_UNUSED;
			}
		}
	}
}
...
int main(void)
{
	...
	/* Set the file indices to UNUSED */
	memset(lc3_file_idx, LC3_FILE_INDEX_UNUSED, sizeof(lc3_file_idx));
	...
	for (size_t i = 0; i < CONFIG_BT_ISO_MAX_BIG; i++) {
		...
		for (size_t j = 0; j < broadcast_param[i].num_subgroups; j++) {
			for (size_t k = 0; k < broadcast_param[i].subgroups[j].num_bises; k++) {
				...
				uint8_t streamer_idx = lc3_file_idx[i][j][k];
...
static int cmd_preset(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!preset) {
		shell_error(shell, "Preset not found, use 'bct preset print' to see "
				   "available presets");
		return -EINVAL;
	}
...
static int cmd_file_list(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = sd_card_list_files(NULL, buf, &buf_size);
	if (ret) {
...
static int cmd_file_select(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (argc < 4) {
		shell_error(shell, "Usage: bct lfile_select <file> <BIG index> <subgroup index> "
				   "<BIS index> [loop file]");
		return -EINVAL;
	}
	...
	LOG_ERR("Selecting file %s for stream %d.%d.%d", file_name, big_idx, sub_idx, bis_idx);

	ret = lc3_streamer_stream_register(file_name, &lc3_file_idx[big_idx][sub_idx][bis_idx],
					   loop_file);
	if (ret) {
	...
	LOG_ERR("Index is %d", lc3_file_idx[big_idx][sub_idx][bis_idx]);
...
static int cmd_framing(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (argc != 4) {
		shell_error(shell, "Usage: bct framing <unframed/framed> <BIG index> "
				   "<subgroup index>");
		return -EINVAL;
	}
...
static int cmd_frame_interval(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (argc != 4) {
		shell_error(shell, "Usage: bct frame_interval <time in us> <BIG index> "
				   "<subgroup index>");
		return -EINVAL;
	}
...
static int cmd_send(const struct shell *shell, size_t argc, char **argv)
{
	...
	audio_send((void *)data, sizeof(data), 1);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_COND_CMD(CONFIG_SHELL, send, NULL, "List files on SD card", cmd_send),
